...
def evaluate(t_rc, camera_rb_poses, all_image_coords, all_object_coords, t_co, camera_mat, distortion_coeffs):
    ...
    for t_mo, image_coords, object_coords in zip(t_mos, all_image_coords, all_object_coords):
        ...
        print(np.mean(np.linalg.norm(projected_image_coords - image_coords, axis=1)))
...
def evaluate_k_fold(marker_calibration_file, board_calibration_file, raw_data_file):
    ...
    for test_bags, marker_calibration, board_calibration in zip(all_test_bags, marker_calibrations, board_calibrations):
        test_mask = np.logical_not(np.in1d(bag_files, test_bags))
        ...
        evaluate(marker_calibration, test_cam_rb_poses, test_image_coords, test_object_coords, t_co, camera_mat, distortion_coeffs)
        print("-" * 80)
        evaluate(board_calibration, test_cam_rb_poses, test_image_coords, test_object_coords, t_co, camera_mat, distortion_coeffs)
        print("*" * 80)


if __name__ == "__main__":
    evaluate_k_fold("../data/marker_calibration.npz", "../data/board_calibration.npz", "../data/5_marker_data.npz")
